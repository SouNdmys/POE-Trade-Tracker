...
- **B-2 库存数字没有合理性约束**：**已修 2026-09-06，见 `240765e`（书）、`ea54f15`（页面）**。
  汇率有单调序 + 中位带，库存只有 `NoStock`。一位误读（1855 读成 18550）直接进覆盖度、
  流动性档位与雷达排序（流动性是第一排序键）。落地形态：`stock_outlier_quote_ids` 借用
  汇率带的骨架（同侧分桶、元素选中位、不走浮点），同侧不足 3 行**不给任何判断**，
  超出同侧中位数 `STOCK_OUTLIER_FACTOR = 10` 倍（两个方向）的行只挂
  `QuoteRiskFlag::StockOutOfBand`——**不进 `execution_blockers`、不进
  `selection_rejections`**，`accepted_for_selection` 照旧为真。常量写死在
  `ptt-market-book`，**不进设置页**。旗标经 `depth.rs`（只在**真正吃到**的档位上抬）
  → `ExecutionRiskFlag::StockOutOfBand` → `absorb_execution_flag` 归到
  `ModelCaveat::StockOutOfBand`（**不是 `ExecutionRisk`，所以不闸路线**），
  在 Monitor 页显示成一枚 chip。设计理由见 `CORE-TRADING-MODEL.md`
  「0.5.5 开服夜」第一条。
- **B-3 前缀差一字的通货被认成另一个**（崇高石 / 完美崇高石）：**首版已落地 2026-09-06，
  见 `291927c`（判据）、`903138f`（接进管道）**。精确匹配挡不住"合法命中"，书落错对
  并污染历史。落地形态：落库前取本书**吃单侧精确行里最好的那个**汇率（汇总行 `<`/`>`
  排除，和日折 `top_taker_rate` 同一口径——两侧算法不一样，十倍这条带在两边就是两个
  意思），对这对通货**最近 3 天里有数据的最后一天**的日折 `median_top_taker_rate`，
  差过 `IDENTITY_SANITY_FACTOR = 10` 倍就发一条 `PipelineEvent::Warning`
  （走 `report_text` 双语目录，落进 Monitor 页现有的日志行，不是故障横幅），
  **书照常落库**。判据借 `Ratio::differs_by_more_than`（整数交叉相乘，不走浮点）。
  读不到东西的每一种情况——没有 rollup、这对没历史、这本书没有精确吃单行——**一律放行**。
  为了让集成测试走生产路径而不是复刻一遍，把这一步抽成了 `warn_then_persist`，
...
- **B-5 「>3h 空小时永久标记」会把发布延迟固化成永久空洞**：**已修 2026-09-06，见
  ...
  `a374c99`（页头计数）、`50a23ce`（计数起点）**。实测 API 延迟本来就 1–2 小时。
  落地形态：
...
- **联赛名改成下拉 + 仍可手填**：**已修 2026-09-06，见 `b7a263d`（记全 + 持久化）、
  ...
  旧设置文件照常加载、列表为空），且**只在非空时写**——一轮只看到"还没发布"的小时并不
  说明联赛没了，不能拿它抹掉上一轮的好名单。装不下的问题挪到显示侧：日志行和页面提示
...
0.5.5 开服夜一口气落了 12 个 commit，其中五条改的是**判定规则**。规则改了不等于数字对了
...
- **B-2 的 10×（`STOCK_OUTLIER_FACTOR`）要不要升为设置页旋钮**。现在刻意写死，理由是设置页
  ...
  对账的带。等有一整季的抓取样本再决定。
- **库存 0 的行也会被点名**（`median > 10 × 0` 恒真）。判据本身不够干净。目前不可见：
  这些行早就被 `NoStock` 拒了，而深度走只在**真正吃到**的档位上抬旗标，走不到它们。
  真要修是在 `stock_outlier_quote_ids` 里给 `stock == 0` 一条早退——但那和 `NoStock`
  的职责边界要先想清楚，不是顺手改。
