...
- **雷达带里的天文涨幅**：`預知代價 +48440943%`。开服头几天神圣石极稀缺，以神圣计价的
  卡牌价格是天文数字，两天前后一比就爆表。显示已封顶（`>+999%`，见 exchange.rs
  `signed_percent`），但入选逻辑没动：雷达带该过滤掉**基线成交太薄**的资产
  （基线那几天的锚计价成交额低于某个门槛就不评涨跌），门槛等新赛季跑出一周
  的活体数据再定。
...
- **B-2 库存数字没有合理性约束**：汇率有单调序 + 中位带，库存只有 `NoStock`。一位误读
  直接进覆盖度、流动性档位与雷达排序（流动性是第一排序键）。方向：同侧库存相对中位数的
  粗带（10×）旗标而不剔除。
- **B-3 前缀差一字的通货被认成另一个**（崇高石 / 完美崇高石）：精确匹配挡不住"合法命中"，
  书落错对并污染历史。方向：跨帧校验"这本书的汇率量级和这对通货的历史像不像"。
- **B-5 「>3h 空小时永久标记」会把发布延迟固化成永久空洞**：实测 API 延迟本来就 1–2 小时。
  方向：确认空改成可过期的软标记（24 小时后允许重访一次），或至少在交易所页显示
  "本窗口内有 N 个确认空小时"。
...
- **联赛名改成下拉 + 仍可手填**（已拍板形状）：下拉列出上一轮同步在数据里见到的所有联赛名
  （`SyncRound.leagues_seen`，现在只取前 5，改成全部并持久化到设置里），含 HC 与私人联赛
  PLxxxxx；选一个即保存并重启同步；列表为空（从没同步过）时退回文本框。
  理由：官方小时文件装着该游戏所有联赛，程序按名字逐字精确筛（`ptt-exchange-history/src/lib.rs`
  `rows_for_league`），名字就是"选服务器"，但不该让用户凭空拼。
- **"全部保留"档的成交量封顶在 30 天**（`reports.rs` 约 3620 行把 `retention_days` 夹在 1..30；
  `shell/mod.rs` 约 2149 行调用），用户设 45 天保留时表头写"全部保留"实际只算 30 天小时明细，
  而同行的走势小图用日线画满全部天数。二选一：按实际保留天数算，或把 chip 文案改成真实跨度。
