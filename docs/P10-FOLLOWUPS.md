...
- ~~**「价值」「深度」整列是「–」而右栏账本有数**~~ **已修 2026-09-06，见
  `exchange page: the hourly columns count back from the sync watermark, like the ledger does`**：
  表里的小时级列看的是「现在往前 48 小时」，账本窗口是「同步水位往前」。水位落后
  166 小时时一边有数一边没数。用户拍板：小时级列也以水位为终点——页头已经写着落后
  多少小时，不会冒充现在。落地形态与原记账写的不同：那 48 小时窗口**不在**
  `exchange_model` 里，而是各调用方自己拿 `now` 算的读库区间（app 的 `load_exchange`、
  探针 `--series`），所以改的是把窗口收进 `reports.rs` 的 `exchange_hour_window(watermark)`
  一处，`exchange_model` 多收一个 `watermark` 参数并按它过滤，两个调用方都用同一个函数
  算读库区间。没有水位（从没同步过）= 没有窗口 = 小时侧照旧空着。
  **交易所雷达（`exchange_radar_model`）没跟着改**：它是另一个函数，自带 24 小时
  `latest_market_hours` 的新鲜度门槛，还要拿 `now` 给闭环判新鲜度红黄绿，
  换终点等于改它的新鲜度语义——单独一条待拍板，见下。
- **交易所雷达在同步落后的夜里同样会整段空白**（2026-09-06 记，随上一条发现）：
  `load_exchange_radar` 读的还是「现在往前 48 小时」，`exchange_radar_model` 里再套一层
  `latest_market_hours(now, EXCHANGE_RADAR_MAX_AGE_HOURS = 24)`。水位落后超过 24 小时，
  雷达就没有可用的市场小时，页面显示「市场不足」。要不要也改成以水位为终点**待拍板**，
  因为这一段和表格不同：它给的是可执行的环路，还要拿 `now` 判新鲜度红黄绿，
  「以水位为终点」等于承认「这条路线是 166 小时前的样子」——那到底该不该出现在
  一个讲机会的段落里，是个语义题，不是窗口题。
