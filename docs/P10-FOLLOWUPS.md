...
**能做的是补抓**：把这些对子的**反方向面板**也翻一次，正向 taker 盘口有了真实数据，
`ReverseFromCompeting` 就消失，可执行性列才会重新有区分度。
...
1. **反方向面板没抓过**，所以每一行都带「竞争方参考价」、可执行性列恒为「要等人吃单」
   （详见第 32 条那段否定结论）。这是**抓取覆盖**问题，代码改不了——去把这些对子的
   反方向翻一次。
