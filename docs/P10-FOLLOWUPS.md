...
## 9. 兑换页负收益路线（排序键已修 2026-08-23，见 `70852e1`、`073c82e` 与同日移除覆盖键的那次改动；按腿的吃下覆盖度已上线；对照行与基准半仍未做）
...
- **数据半（诚实的部分，显示已做一半）**：直兑 taker 深度只有 1855 神聖石（5000 的
  ...
  同页并排不加区分是误读的直接来源。
  **2026-08-23 上线了一半**：最佳路线的每一条腿现在都打一行「市面挂着 X，
  这一趟要吃掉 Y（Z%）」，分母只算**该方向上 taker 执行类型的行**，三档上色。
  裁定与完整理由（为什么分母必须是这条腿而不是这个通货、3,133 vs 11 那个实例、
  为什么没用 `listed_liquidity`、为什么另立一个阈值设置项）在
  `CORE-TRADING-MODEL.md`「按腿的「立刻吃下的覆盖度」」。**它只是 taker 那一半**；
  挂单侧的“会不会卡住”是另一件事，见第 22 条。
  **仍未做的是对侧那一半**：37404 那个数仍然不带区分地并排在旁边。
- **bug 半（主因，已修）**：`compare_paths`（ptt-trade-engine `route.rs`）现在是
  可执行 → **中途搁浅比例** → **已实现单价** → 残余腿数 → 跳数 → 资产名。单价键先把
  amount_out 绝对值换成 `amount_out / 首腿 consumed_input`（吃满的路线之间序不变，分母
  相同即退化回原来的比较），随后在它**前面**插入流动性闸：首腿以外每条腿"没吃下去 ÷
  交到它手上"的最差比例，小的在前；最后（2026-08-23 第二次裁定）把首键的覆盖度
  `is_fully_filled` 整个拿掉。裁定与完整理由——包括"为什么数量根本不该进排序键"、判据
  为什么只看中间腿、为什么不能复用雷达那套阈值、覆盖键被否的反例、以及被否掉的其余排序
  键——在 `CORE-TRADING-MODEL.md`「兑换路线排序：流动性 > 兑换率，数量不参与」与同文档
  「否掉的方案 · 排序与风险」。回归测试 `compare_paths_tests`（`route.rs` 底部，八条）。
...
1. 把直兑作为**对照行**并排列出，让"直兑只能吃 1855"这个真约束可见。
2. 文案说明绕路代价；`compare_best_to_direct` 的 IncomparableCoverage 保护与
   convert_model 绕过它这件事；直兑的线性外推基准。
3. 流动性闸只认**已经发生的**搁浅（吃满的腿看不出后面还剩多少余量，见 CORE 文档）。
   要让它也能判"这条路各腿本来就薄"，得把整侧的 `listed_stock` 顺着 `PairFill` 带出
   `MarketDepthIndex`，那是一次独立改动，今天没做。
   **2026-08-23 补**：页面那一层现在能看到池子了（下面那行按腿的覆盖度），
   但 `compare_paths` 看不到：它是纯比较函数，手上没有 `MarketDepthIndex`。
   **裁定：搃浅闸与新指标并存**，一个替代不了另一个——前者是观测值（只含
   真的没吃下去的部分，两条都吃满的路线在它那里恒平局），后者含深度行走没
   走到的挂单（能看见“两条都吃满、但其中一条本来就只剩一档”）。两者合并与否，
   等这次 `listed_stock` 带出引擎的改动落地了再拍。
4. **“>100% 参与排序粗筛”未落地，因为落点上没有排序可参与**（新增 2026-08-23）：
   `convert_model` 每个尺寸只有**一条**最佳路线（`compare_paths` 选出来的），页面
   自己不排序。真要让这一档压后，两条路：在页面这层从 `alternatives` 里换一条
   （= 仓内多一个路线仲裁者，与“排序只归 `compare_paths`”冲突），或者等上一条的
   `listed_stock` 带出引擎。本次只显示、不改选路。
5. **`execution_eligible` 里还藏着一个尺寸键（待裁定）**。它的合取项之一就是
