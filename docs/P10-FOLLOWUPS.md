...
- **bug 半（主因，已修）**：`compare_paths`（ptt-trade-engine `route.rs`）现在是
  覆盖 → 可执行 → **中途搁浅比例** → **已实现单价**。单价键先把 amount_out 绝对值
  换成 `amount_out / 首腿 consumed_input`（吃满的路线之间序不变，分母相同即退化回
  原来的比较），随后在它**前面**插入流动性闸：首腿以外每条腿"没吃下去 ÷ 交到它手上"
  的最差比例，小的在前。裁定与完整理由——包括"为什么数量根本不该进排序键"、判据为什么
  只看中间腿、为什么不能复用雷达那套阈值、雷达页为什么不受影响、以及被否掉的三个排序
  键——在 `CORE-TRADING-MODEL.md`「兑换路线排序：流动性 > 兑换率，数量不参与」与同文档
  「否掉的方案 · 排序与风险」。回归测试 `compare_paths_tests`（`route.rs` 底部，六条）。
